...
@Header("CFDate")
@OpaqueType("CFDateRef")
public class CoreDate implements Comparable<CoreDate>, CorePropertyList, CoreType {
    private final double mAbsoluteTime;

    @CFunction("CFDateCreate")
    public CoreDate(double secondsSince2001) {
        mAbsoluteTime = secondsSince2001;
    }

    @CFunction("CFDateGetTimeIntervalSinceDate")
    public double getTimeIntervalSinceDate(CoreDate otherDate) {
        return mAbsoluteTime - otherDate.getAbsoluteTime();
    }

    @CFunction("CFDateGetAbsoluteTime")
    public double getAbsoluteTime() {
        return mAbsoluteTime;
    }

    @CFunction("CFDateCompare")
    @Override
    public int compareTo(CoreDate other) {
        double d = mAbsoluteTime - other.getAbsoluteTime();
